...
public class ValueReader {
	...
	public static EncodedValue readEncodedValue(RandomAccessFile raf, DexContainer dc) throws IOException
	{
		...
		switch(ev.type)
		{
			...
			{
				((EncodedValue.ByteValue)(ev.valueContainer)).value = raf.readByte(); break;
			}
			case(EncodedValue.VALUE_SHORT) :
			{
				((EncodedValue.ShortValue)(ev.valueContainer)).value = (short)GeneralUtil.readBytes(raf, ev.size); break;
			}
			case(EncodedValue.VALUE_CHAR) :
			{
				((EncodedValue.CharValue)(ev.valueContainer)).value = (char)GeneralUtil.readBytes(raf, ev.size); break;
			}
			case(EncodedValue.VALUE_INT) :
			{
				((EncodedValue.IntValue)(ev.valueContainer)).value = (int)GeneralUtil.readBytes(raf, ev.size); break;
			}
			case(EncodedValue.VALUE_LONG) :
			{
				((EncodedValue.LongValue)(ev.valueContainer)).value = GeneralUtil.readBytes(raf, ev.size); break;
			}
			case(EncodedValue.VALUE_FLOAT) :
			{
				((EncodedValue.FloatValue)(ev.valueContainer)).value = Float.intBitsToFloat((int)(GeneralUtil.readBytes(raf, ev.size))); break;
			}
			case(EncodedValue.VALUE_DOUBLE) :
			{
				((EncodedValue.DoubleValue)(ev.valueContainer)).value = Double.longBitsToDouble(GeneralUtil.readBytes(raf, ev.size)); break;
			}
			...
			{
				ev.valueContainer = readEncodedArray(raf, dc); break;
			}
			case(EncodedValue.VALUE_ANNOTATION) :
			{
				ev.valueContainer = AnnotationReader.readEncodedAnnotation(raf, dc); break;
			}
		}
