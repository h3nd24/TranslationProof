...
public class DebugSummary {
	...
	public DebugSummary(String owner_file, short start_line, short register_size, int opcode_size, DebugOpCode[] opcodes)
	{
		...
		for (int i = 0; i < opcode_size; i++)
		{
			...
			switch(opcodes[i].opcode_raw)
			{
				...
				case (DebugInfoItem.DBG_SET_PROLOGUE_END) : { break;} // not yet used
		...
		for (int i = 0; i < register_size; i++) 
		{
			if (!registers[i].register_closed) register_scopes.get(i).get(registers[i].scope_size - 1).value_2 = current_address;
	...
	public String toString()
	{
		...
		for (int i = 0; i < location_size; i++) sb.append(indent + "\t" + GeneralUtil.shortToHex(locations[i].value_1) + " line=" + locations[i].value_2 + "\n");
