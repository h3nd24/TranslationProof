...
public class DebugInfoItem {
	...
	public static class DebugOpCode
	{
		...
		public boolean hasRegister() {return register_exists;}
		public boolean hasAddress() {return addr_exists;}
		public boolean hasLine() {return line_exists;}
		public boolean hasName() {return name_exists;}
		public boolean hasType() {return type_exists;}
		public boolean hasSig() {return sig_exists;}
		...
		public DebugOpCode(byte opcode_raw, String opcode_str, byte flag, DexContainer dc, int ... args)
		{
			...
			this.flag = flag;
			int args_pos = 0;
			if ((flag & HAS_REG) != 0) {register_exists = true; register_num = args[args_pos++];}
			if ((flag & HAS_ADDR) != 0) {addr_exists = true; addr_diff = args[args_pos++];}
			if ((flag & HAS_LINE) != 0) {line_exists = true; line_diff = args[args_pos++];}
			if ((flag & HAS_NAME) != 0) 
			{
				name_exists = true; 
		...
		public String toString()
		{
			...
			if (flag != 0)
			{
				...
				if (type_exists) 
				{
					if (type_idx != GeneralUtil.NO_INDEX) sb.append("type={" + type.toString() + "} ");
